...
{
	//open file -- 判断同名文件是否存在？？
	FileManager::createFile("tables");
	FileManager::createFile("indices");
	tablesFile = new FileManager("tables");
	indicesFile = new FileManager("indices");
...
//    }
	...
	//write to tablesFile
    string tableN;
    tableN = "table_" + string(tableName);
	
    char tableInfo[MAX_NAME_LENGTH * 2];
	memcpy(tableInfo, tableN.c_str(), MAX_NAME_LENGTH);
	memcpy(tableInfo + MAX_NAME_LENGTH, primary, MAX_NAME_LENGTH);
	int id = tablesFile->addValue(tableInfo);

    
	//create file to store column information before create Table
    
	FileManager::createFile((string(tableN)).c_str());
	FileManager* colFile = new FileManager((string(tableN)).c_str());
	...
	for (int i = 0; i < colNum; i++) {
		...
		tablesFile->addValue(col);
	}


	//record table to map
	tableIdMap[tableN] = id;
	tableMap[tableN] = new Table(tableInfo);
...
//    tablesFile->removeValue(tableIdMap[tableName]);
//
...
bool CatalogManager::createIndex(const char* indexName, const char* tableName, const char* colName)
{
	...
	int id = indicesFile->addValue(indexInfo);
