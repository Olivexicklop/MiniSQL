...
bool Api::insert(const char* tableName, const vector<char*>* value)
{
    ...
    // Parse data
    char* data = new char[table->getRecordLenth()];
    ...
    for (auto index : indices)
        indexManager->insert(
            index->getIndexName(), vec[table->getId(index->getColName())], res
        );
...
)
{
    ...
    Table* table = catalogManager->getTableName(tableName);
    if (table == NULL)
        return false;
    ...
    vector<char*> vec;
    vector<Index*> indices;
    catalogManager->getIndexByTableName(tableName, &indices);
    ...
    for (auto data : record)
    {
        ...
        for (auto index : indices)
            indexManager->remove(
                index->getIndexName(), vec[table->getId(index->getColName())]
            );
...
bool Api::createIndex(const char* indexName, const char* tableName, const char* colName)
{
    ...
    if (catalogManager->createIndex(indexName, tableName, colName))
    {
        ...
        while ((id = file->getNextRecord(data)) >= 0)
        {
            ...
            table->getColValue(colName, data, dataOut);
...
)
{
    ...
    for (int i = 0; i < condCount; i++)
    {
        ...
        if (key == NULL)
            return 0;
        int id = indexManager->find(index->getIndexName(), key);
        ...
        else
        {
            ...
            {
                int recordLength = table->getRecordLenth();
